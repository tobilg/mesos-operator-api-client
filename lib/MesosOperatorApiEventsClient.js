 ...
 */
function MesosOperatorApiEventsClient (options) {

    if (!(this instanceof MesosOperatorApiEventsClient)) {
        return new MesosOperatorApiEventsClient(options);
    }
    ...
    self.allowedEventTypes = [
        "SUBSCRIBED", "TASK_ADDED", "TASK_UPDATED"
    ];
...
// Inherit from EventEmitter
util.inherits(MesosOperatorApiEventsClient, EventEmitter);

/**
 * Subscribe to the Mesos Operator API events
 */
MesosOperatorApiEventsClient.prototype.subscribe = function () {
...
/**
 * Unsubscribes from the mesos Event Bus
 */
MesosOperatorApiEventsClient.prototype.unsubscribe = function () {
    let self = this;
    self.emit("unsubscribed");
    setTimeout(function () {
        //self.httpRequest.close();
        self.emit("close_request");
    }, 100);
};

module.exports = MesosOperatorApiEventsClient;
