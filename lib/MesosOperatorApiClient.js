...
function MesosOperatorApiClient (options) {
    ...
    self.allowedApiMethods = helpers.getSupportedApiMethods();
    ...
    self.allowedApiMethods.forEach(function (apiMethod) {
        // Add supported methods
        self[helpers.getFunctionNameFromApiMethod(apiMethod)] = function (callback) {
            ...
            helpers.doRequest.call(self, requestBody, function (error, response) {
                if (error) {
                    ...
                    self.emit(helpers.getEventNameFromApiMethod(apiMethod), body);
                    // Check if callback is added, if so, trigger it
                    if (callback) {
                        callback(null, body);
                    }
